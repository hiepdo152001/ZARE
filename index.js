...
function getBoard(boardId, name, token) {
  ...
  axios
    ...
    .then((response) => {
      // home me
      profile = response.data.gameObjects.filter(
        (item) =>
          item &&
      ...
      // bot me
      bot = response.data.gameObjects.filter(
        (item) =>
          item && item.type === "BotGameObject" && item.properties.name === name
      );

      // bot friend
      friend = response.data.gameObjects.filter(
        (item) =>
          item &&
      ...
      // home friend
      friendHome = response.data.gameObjects.filter(
        (item) =>
          item &&
          item.type === "BaseGameObject" &&
          item.properties.teamId === bot[0].properties.teamId &&
      ...
      enemys = response.data.gameObjects.filter(
        (item) =>
          item &&
          item.type === "BotGameObject" &&
          item.properties.teamId !== bot[0].properties.teamId
      ...
      enemysHome = response.data.gameObjects.filter(
        (item) =>
          item &&
          item.type === "BaseGameObject" &&
          item.properties.teamId !== bot[0].properties.teamId
      ...
      // list coint on board
      coinGames = response.data.gameObjects.filter(
        (item) => item && item.type === "CoinGameObject"
      );

      //arr point coin
      var ArrCoins = coinGames.map((coin) => ({
        x: coin.position.x,
        y: coin.position.y,
